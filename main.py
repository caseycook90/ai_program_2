...
def fitness(schedule):
    ...
    for activity, details in schedule.items():
        ...
        ◦ If any facilitator scheduled for consecutive time slots: Same rules as for SLA 191
            and SLA 101 in consecutive time slots—see below.
        ...
        (e.g., 10 AM &amp; 11 AM): +0.5
        ◦ In this case only (consecutive time slots), one of the activities is in Roman or
        Beach, and the other isn't: -0.4
        ▪ It's fine if neither is in one of those buildings, of activity; we just want to avoid
        having consecutive activities being widely separated.
        ▪
...
def mutate(schedule, mutation_rate):
    ...
    # randomly choose whether to change the facilitator, room, or time for the course
        ...
        if mutation_type == 'facilitator':
            new_value = random.choice(["Lock", "Glen", "Banks", "Richards", "Shaw", "Singer", "Uther", "Tyler", "Numen", "Zeldin"])
        elif mutation_type == 'room':
            new_value = random.choice(["Frank 119", "Loft 310", "Roman 216", "Logos 325", "Beach 201", "Slater 003", "Beach 301", "Loft 206", "Roman 201"])
